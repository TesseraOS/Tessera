...
- **Theming today:** `next-themes` class-based dark mode only ([providers.tsx](../../apps/web/app/providers.tsx), `defaultTheme="dark"`); tokens in [globals.css](../../apps/web/app/globals.css) `:root`/`.dark` (hex values, efferd monochrome); toggle = header dropdown (`components/theme-toggle.tsx`, replaced by the AppearanceSwitcher); 3 theme commands in the ⌘K palette.
