...
/**
 * DESIGN-LINT — the enforcement half of docs/design/MARKETING-DESIGN.md (ADR-0042).
 *
 * This suite compiles `enforcement.bannedPatterns` / `enforcement.requiredPatterns` from
 * docs/design/marketing-design.manifest.json and fails the standard `test` gate on any
 * violation. If a case fails: FIX THE CODE. Editing a pattern or adding an `allowIn`
 * exemption is a design decision — it requires updating MARKETING-DESIGN.md and the
 * manifest together, and review. Never weaken the contract to make a build pass.
 */
...
const MANIFEST_PATH = join(
  ...
  'docs',
  'design',
  'marketing-design.manifest.json',
);
...
describe('design-lint: banned patterns (MARKETING-DESIGN.md)', () => {
...
describe('design-lint: required patterns (MARKETING-DESIGN.md)', () => {
