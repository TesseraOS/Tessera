				...
				<CardContent className="p-0 pt-4">
					<form onSubmit={submit} aria-label="Compile a context package">
						<div className="grid gap-4 sm:grid-cols-[1fr_auto_auto] sm:items-end">
							...
							<Button type="submit" size="sm" className="h-9 text-xs" disabled={compile.isPending || task.trim().length === 0}>
								<IconPlaceholder
				...
				<Card className="border-none bg-sidebar p-4 shadow-none dark:ring-0" key={`${section.title}-${idx}`}>
					...
					<CardContent className="p-0 pt-4 flex flex-col gap-3">
						{section.fragments.map((fragment, fidx) => (
							<div key={`${fragment.ref}-${fidx}`} className="space-y-2 rounded-xl bg-background/30 border border-border/30 p-3">
								<div className="flex items-start justify-between gap-3">
									...
									<div className="flex shrink-0 items-center gap-1.5">
										...
										<Badge variant="secondary" className="font-mono text-[9px] h-5 px-1.5 tabular-nums">
											{fragment.tokens} tok
										</Badge>
									</div>
								</div>
								...
								<div className="flex flex-wrap items-center gap-1.5 pt-1.5 border-t border-border/30">
									{fragment.provenance.signals.map((signal) => (
										<Badge key={signal} variant="outline" className="font-mono text-[9px] h-5 px-1.5 font-normal">
											{signal}
										</Badge>
									))}
									{fragment.provenance.expandedFrom ? (
										<Badge variant="outline" className="font-mono text-[9px] h-5 px-1.5 font-normal">
											← {fragment.provenance.expandedFrom}
										</Badge>
									) : null}
							...
							{stage.dropped.length > 0 ? (
								<ul className="mt-1.5 space-y-1 bg-background/20 rounded-lg p-2 font-mono text-[10px]">
									{stage.dropped.map((drop, didx) => (
										<li key={`${drop.ref}-${didx}`} className="text-muted-foreground flex flex-col sm:flex-row sm:justify-between gap-1">
