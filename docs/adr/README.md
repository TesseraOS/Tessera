...
| [0056](0056-entitlement-clamp-silent-and-metered-only.md) | The compile entitlement clamp: SILENT on `compile_context` (the clamp is already derivable — `pkg.budget` IS the effective budget) but PUBLISHED in the tool description, named explicitly in `explain`, and applied to METERED deployments only — a self-hosted deployment that wired no `BillingProvider` is not "on the free plan" and is not capped; the cloud Free tier keeps its 8000 | Accepted |
