...
| [0062](0062-apache-2-licence-whole-repo-oss-and-the-publish-closure.md) | Launch licensing + release: **Apache-2.0** over MIT for its express patent grant and its trademark section (ADR-0008 locks the *Tessera* mark and a hosted tier is planned under it — MIT would not stop a fork shipping under the same name); **the whole repository is the open core** and what is sold is the SERVICE, not a carved-out package, because splitting `@tessera/billing` would run a licence boundary through a package that works end to end today; the publish set is the CLI's **computed 18-package closure, not the 3 the F-054 note recorded** — publishing 3 means `npm i -g @tessera/cli` cannot resolve `@tessera/api` and fails, a break no build/typecheck/test in this repo can see because `workspace:*` always resolves inside the workspace; and releases are `workflow_dispatch` with **`publish: false` by default**, since a tag-triggered first release is an irreversible publish of names npm will not let you reuse | Accepted |
| [0063](0063-executable-parity-over-screenshots-and-an-enumerated-i18n-allowlist.md) | F-064's two acceptance clauses that said the wrong thing, both escalated rather than reinterpreted: the requested **screenshot matrix** is replaced by executable parity assertions (13 routes × mobile/desktop must not overflow; reduced-motion asserted on COMPUTED durations, since the media query can be present and not apply — and Chromium reports the override as `1e-05s`, so parse rather than string-match), because F-057 already rejected screenshots for this job and its reasoning is still in the code; and **i18n ships as catalog + typed `t()` + an enforced ESLint guard + an ENUMERATED allowlist** rather than one 52-file sweep, because a wrong string renders as a blank with no test failing — a risk proven mid-migration when a catalog value seeded from a truncated lint message silently lost half a sentence. Flat keys (nesting kills the key union), no fallback-to-key (a fallback makes a missing message look shipped), and a deliberately narrow rule because a noisy one gets disabled | Accepted |
