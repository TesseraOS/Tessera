...
14. **`legal-prose`** (v4.10, F-067 v2 — the `/legal/*` treatment, five routes incl.
    `gdpr`) — **expressive frame, calm document.** **Opening:** the §3.12 `page-header`
    hero (full-height shader ground): eyebrow (`legal — privacy`) · serif `title`-token
    h1 · lead ≤56ch from the `LegalDoc` · the "last updated" line (`<time>`) + a `Badge`
    reading *preliminary — final on incorporation* as children · a bespoke **legal
    signature art** in the art slot (§4 `LegalArts`: RedactionGate / TwoCovenants /
    OneTile / Nameplate / RightsLedger — honest product truths in the house art
    language, all §5 motion rules). v4.9's compact quiet opening is RETIRED — its
    recorded fallback (PageHeader reuse), promoted and extended with per-page art.
    **Body (unchanged from v4.9):** a single-column `max-w-prose` article (~65ch); h2s
    serif via the base layer on the **`heading`** size at weight 400 (the serif never
    bolds); h3s `text-body font-medium`; lists; **token tables** (hairline `border`,
    `text-small` cells, `th scope="col"`, visible caption). **`CounselReview`
    placeholder:** every unresolved entity/jurisdiction/contact fact renders as a
    visually distinct callout — dashed `border-strong`, `bg-surface`, `text-label`
    eyebrow "pending counsel review", `<aside role="note" aria-label="Pending counsel
    review">` — spending **zero accent**. **NO fabricated legal facts and never a
    GDPR-compliance claim** — placeholders, never prose, carry the unknowns. The
...
only, v4.9; v4.10 body-only, the page composes the hero) · **`LegalArts`** (five legal
signature arts, v4.10 — RedactionGate: privacy, secret glyphs mask at the gate;
TwoCovenants: terms, one engine core under open field + managed canopy; OneTile:
cookies, a near-empty shelf whose single `theme` tile lights only when touched;
Nameplate: imprint, the seal assembles beside engraved slots left visibly empty;
RightsLedger: gdpr, request in · export out · erased tile dissolves — motion-seam only,
`role="img"`, reduced-motion still scenes) · `Wordmark` ·
